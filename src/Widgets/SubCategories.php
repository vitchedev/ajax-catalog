...
class SubCategories extends Widgets
{
    ...
    */
    private function getGroupedValues()
    {
        ...
        //get all necessary parent values
        $parentFilter = $this->urlData->settings['filters'][$this->system_name]['parent_filter'];


        $parentValue = '';
        foreach ($this->urlData->params['filters'] as $filter) {
            if ($filter[3] == $parentFilter) {
                $parentValue = $filter[2];
            }
        }

        $parentValue2 = $this->values[$this->getAlias($parentValue)];
        $parents = array_flip($this->getAllParentValues($parentValue2));
        $parents[$parentValue] = 0;

        //get full category to work with
        foreach ($this->values as $value) {
            if ($value['parent'] == null && isset($parents[$value['value']])) {
                $grouped_values[$value['alias']] = $this->attachChildValues($value);
            }
        }

        //remove all top elements
        $sub_categories = $this->getSubCategories($grouped_values, $parents);

        return $sub_categories;
    }

    private function getSubCategories($grouped_values, $parents)
    {
        $sub_categories = [];

        foreach ($grouped_values as $group_val) {
            if (isset($parents[$group_val['value']])) {
                unset($parents[$group_val['value']]);
                if (isset($group_val['children'])) {
                    if (count($parents) > 0) {
                        $sub_categories = $this->getSubCategories($group_val['children'], $parents);
                    } else {
                        $sub_categories = $group_val['children'];
                    }
                }
            }
        }
